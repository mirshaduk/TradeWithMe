...
export default function Home() {
    ...
    const [autoTradeEnabled, setAutoTradeEnabled] = useState(true); // ON by default
    ...
    const handleTradeLog = useCallback((entry) => {
        setTradeLog(prev => [{ ...entry, time: new Date().toLocaleTimeString() }, ...prev].slice(0, 20));
    }, []);
    ...
    return (
        <>
            {/* Headless AI Auto-Trader — runs in background */}
            <AutoTrader enabled={autoTradeEnabled} onLog={handleTradeLog} />
                ...
                <main className="flex-1 flex flex-col p-4 lg:p-6 space-y-6 overflow-y-auto w-full relative z-0">
                    ...
                    <header className="flex items-center justify-between glass-card p-4">
                        ...
                        <div className="flex gap-4 items-center">
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-bold uppercase tracking-wider transition-all shadow-md ${autoTradeEnabled
                                    ? 'bg-primary/20 text-primary border-primary/40 shadow-[0_0_15px_rgba(108,99,255,0.2)]'
                                    : 'bg-surface text-textMuted border-border hover:border-primary/30 hover:text-primary'
                                    }`}
                                title={autoTradeEnabled ? 'AI Auto-Trading is ON — click to pause' : 'AI Auto-Trading is OFF — click to activate'}
                            >
                                {autoTradeEnabled ? <Bot className="w-3.5 h-3.5" /> : <BotOff className="w-3.5 h-3.5" />}
                                <span className="hidden sm:inline">{autoTradeEnabled ? 'AI Trading ON' : 'AI Trading OFF'}</span>
                                {autoTradeEnabled && <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />}
                            </button>
                        </div>
                    </header>
                    ...
                    <div className="flex flex-col xl:flex-row gap-6 flex-1 min-h-[700px]">
                        ...
                        <div className="flex-1 flex flex-col gap-6">
                            ...
                            {/* AI Trade Activity Log */}
                            <div className="glass-panel rounded-2xl overflow-hidden border border-white/5 shadow-inner">
                                <div className="flex items-center justify-between p-3 border-b border-border/50 bg-surface/80">
